...
fun decode(str, moveAmt) =
  let
	...
	in
   		if (String.size(str1) = 0) then Char.toString(chr(charInt))
   		else if ((charInt + moveAmt) = 32) then String.^(Char.toString(chr(32)), decode(str1, moveAmt))
   		else if ((charInt + moveAmt) < 65) then String.^(Char.toString(chr(charInt + moveAmt)), decode(str1, moveAmt))
		else if (charInt < 64) then String.^(Char.toString(chr((90 - (64 - charInt)))), decode(str1, moveAmt))
